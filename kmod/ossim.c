...
static void free_vcpu_info_rcu(struct rcu_head *head) {
  struct ossim_vcpu_info *vcpu =
      container_of(head, struct ossim_vcpu_info, rcu);
  kfree(vcpu);
}
...
static int do_unregister_vcpu(pid_t __user *arg) {
  ...
  if (vcpu->qemu_pid != current->tgid) {
    spin_unlock(&vcpu_registry_lock);
    pr_err("[ossim] Unregister permission denied: tid=%d registered by pid=%d, "
           "caller is pid=%d\n",
...
static int do_update_vcpu(struct ossim_vcpu_registration __user *arg) {
  ...
  /* Re-export to BPF map (outside lock to avoid holding spinlock during
   * syscall) */
  ret = export_to_bpf_map(vcpu);
...
static int ossim_proc_vcpus_show(struct seq_file *m, void *v) {
  ...
  seq_printf(m, "%-8s %-8s %-20s %-6s %-6s %-8s %-8s\n", "TID", "QEMU_PID",
             "VM_NAME", "INDEX", "FLAGS", "ENQUEUES", "DISPATCHES");

  rcu_read_lock();
  hash_for_each_rcu(vcpu_registry, bkt, vcpu, hlist) {
    seq_printf(m, "%-8d %-8d %-20s %-6u 0x%-4x %-8llu %-8llu\n", vcpu->vcpu_tid,
               vcpu->qemu_pid, vcpu->vm_name, vcpu->vcpu_index, vcpu->flags,
...
static int __init ossim_init(void) {
  ...
  ossim_proc_vcpus =
      proc_create("vcpus", 0444, ossim_proc_dir, &ossim_proc_vcpus_ops);
  if (!ossim_proc_vcpus) {
  ...
  ossim_proc_stats =
      proc_create("stats", 0444, ossim_proc_dir, &ossim_proc_stats_ops);
  if (!ossim_proc_stats) {
